...
import 'package:spam2/component/channel/IOSSpamBlockerManager.dart';
...
class IOSStatusScreen extends ConsumerStatefulWidget {
  final Function(bool loading) loading;
  const IOSStatusScreen({super.key, required this.loading});

  @override
  createState() => _IOSStatusScreenState();
}

class _IOSStatusScreenState extends ConsumerState<IOSStatusScreen> {

  _download() async {
    final list = await SearchService().findAll();
    await SpamBlockerManager().updateSpamNumbers(list);
  }
  ...
  _init() async {
    final isEnabled = await IOSCallKitService().isCallDirectoryExtensionEnabled();
    print('IOS CallDirectoryExtension is Enabled? = $isEnabled');
  }
  @override
  void initState() {
    _init();
